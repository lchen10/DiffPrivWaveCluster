...
//		print2DArray(cost, "%.1f\t", "\nNow the cost matrix becomes:", 0);
	...
	public static void main(String[] args) throws IOException {
		...
		FileWriter fstream = new FileWriter("E:/D_disk/workspace/WaveCluster_PrivTHREM/data_experiments/Dissimilarity_2.txt");
		...
		// E:\Clusters\output
		File outputFolder = new File("E:/D_disk/workspace/WaveCluster_PrivTHREM/data_experiments");
		...
		int count = 0;
		int SumOfTotalPrivateGrids = 0;
		int SumOfTotalTrueGrids = 0;
		for (Entry<String, String> entry : filemap.entrySet()) {
			count++;
			boolean isTransposed = false;
			
			String trueFileName = "E:/D_disk/workspace/WaveCluster_PrivTHREM/data_experiments/" + entry.getKey();
			String privateFileName = "E:/D_disk/workspace/WaveCluster_PrivTHREM/data_experiments/" + entry.getValue();
...
//			System.out.println("private: " + privateFileName);
			...
			int totalTrueGrids = 0;
			for(int s = 0; s < trueClusters.size(); s++)
			{
				int EachClusterSize = trueClusters.get(s).size();
				totalTrueGrids += EachClusterSize;
				
			}
			
			
			out.write("Total True Grid is: " + totalTrueGrids + "\n");
			SumOfTotalTrueGrids += totalTrueGrids;
			
			
			int totalPrivateGrids = 0;
			for(int s = 0; s < privateClusters.size(); s++)
			{
				int EachPrivateClusterSize = privateClusters.get(s).size();
				totalPrivateGrids += EachPrivateClusterSize;
				
			}
			out.write("Total Private Grid is: " + totalPrivateGrids + "\n");
			SumOfTotalPrivateGrids += totalPrivateGrids;
			
			
			for(int x = 0; x < trueClusters.size(); x++)
			{
				int idx = x+1;
				System.out.println("Truecluster #" + idx + ", size is: " + trueClusters.get(x).size());
			...
			{
				int idx = y+1;
				System.out.println("Privatecluster #" + idx + ", size is: " + privateClusters.get(y).size());
			...
			if(dist.length == 1 && dist[0].length == 1)
			{
				...
				out.write("TrueClusters, count of points is " + trueClusters.get(0).size() + " : " + trueClusters.get(0).toString() + "\n");
				out.write("PrivateClusters, count of points is " + privateClusters.get(0).size() + " : " + privateClusters.get(0).toString() + "\n");
				...
				out.write("The sum of matched and non-matched is: " + dist[0][0] + ".\n");
				continue; 
			}
			...
			// {3.1, 2.828},
			// // {3.1622776601683795, 2, 1.4142135623730951}
			// };

			// Another example matrix
			
//			  double[][] array = { {23, 27, 42, 55, 97}, {14, 94, 5, 77, 22},
//			  {53, 3, 88, 89, 7}, {99, 52, 14, 1, 63}, {50, 100, 34, 41, 92} };
			 
//			  double[][] array = { {1,3,3}, {3,3,1}, {3,1,3} };
...
//					("The initial " + arrayType + " array was:"), 1);
			...
			for (int i = 0; i < assignment.length; i++) {
				// System.out.println("assignment[i][0] is: " +
				// assignment[i][0]);
				// System.out.println("assignment[i][1] is: " +
				// assignment[i][1]);
				// System.out.println("assignment[i][1] is: " +
				// assignment[i][0]);
...
//							+ sum);
			...
			double ratio = sum / totalTrueGrids;
			out.write("The dissimilarity 2 ratio is: " + ratio + "\n\n");
		...
		double avgTrueGrid = SumOfTotalTrueGrids / count;
		double avgPrivateGrid = SumOfTotalPrivateGrids / count;
		double avgDissimilarityRatio = totalRatio / count;
		out.write("=======================================================\n");
		
		out.write("The True Grid is: " + avgTrueGrid + "\n");
		out.write("The average PrivateGrid is: " + avgPrivateGrid + "\n");
		
		out.write("The average Dissimilarity 2 ratio is: " + avgDissimilarityRatio + "\n");
		out.flush();
		out.close();
		fstream.close();
	}
