...
public class UniformRandom {
	public static Tuple getRandomTuple(double xmin, double xmax, double ymin, double ymax,
			double delta) {
...
//			System.out.println("x_loc after is: " + x_loc);
		...
		double x = 0.0;
		double y = 0.0;
		if (xmax >= xmin && ymax >= ymin)// first case: (xmin, ymin) is at the lower left of (xmax, ymax)
		{
			x = x_loc * delta + xmin;
			y = y_loc * delta + ymin;
		}
		else if (xmax <= xmin && ymax <= ymin)// second case: (xmax, ymax) is at the lower left of (xmin, ymin)
		{                                   // switch to the case: (xmin, ymin);(xmax, ymax)=>(xmax, ymax);(xmin, ymin)
			x = x_loc * delta + xmax;
			y = y_loc * delta + ymax;
		}
		else if (xmin >= xmax && ymin <= ymax)// third case: (xmin, ymin) is at the lower right of (xmax, ymax)
		{                                   // switch to the case: (xmin, ymin);(xmax, ymax)=>(xmax, ymin);(xmin, ymax)
			
			x = x_loc * delta + xmax;
			y = y_loc * delta + ymin;
		}
		else if (xmin <= xmax && ymin >= ymax)// fourth case: (xmax, ymax) is at the lower right of (xmin, ymin)
		{                                   // switch to the case: (xmin, ymin);(xmax, ymax)=>(xmin, ymax);(xmax, ymin)
			x = x_loc * delta + xmin;
			y = y_loc * delta + ymax;
		}
		else
